...
##NOTE- Ammended file for the 5 species with two wing regions and two tail regions. Before running, create two subdirectories
#Normal - which has all regions but wing, and "TAILAndWIng" which has both wing and tail regions.
...
spp3<-gsub(".\\d+.[A-z]+_\\d+\\.3$",'.zzthroat',spp2,perl=T)
spp4<-gsub(".\\d+.[A-z]+_\\d+\\.4$",'.zzchest',spp3,perl=T)
spp5<-gsub(".\\d+.[A-z]+_\\d+",'.zzcrown',spp4,perl=T)
...
sppspec<-aggspec(mspecs,by=spp5,FUN=mean)
...
out <- list(Euc.crown,Euc.back,Euc.rump,Euc.throat,Euc.chest)
...
export<-as.data.frame(Eucledian.distance, row.names = c("crown","back","rump","throat","chest","Average"), optional = FALSE)
export
one<-getwd()
...
spp<-gsub(".\\d+.[A-z]+_\\d+\\.1$",'.zztailcolour',names(mspecs),perl=T)
spp2<-gsub(".\\d+.[A-z]+_\\d+\\.2$",'.zzwingblack',spp,perl=T)
spp3<-gsub(".\\d+.[A-z]+_\\d+\\.3$",'.zzwingcolour',spp2,perl=T)
spp4<-gsub(".\\d+.[A-z]+_\\d+",'.zztailblack',spp3,perl=T)

table(spp4)

sppspec<-aggspec(mspecs,by=spp4,FUN=mean)
round(sppspec[1:5,],2) 
explorespec(sppspec,by=4,lwd=3)
...
Euc.wingblack<-dist(subset(tcs.all,"zzwingblack")[,c('u','s','m','l')])
Euc.wingcolour<-dist(subset(tcs.all,"zzwingcolour")[,c('u','s','m','l')])
Euc.tailblack<-dist(subset(tcs.all,"zztailblack")[,c('u','s','m','l')])
Euc.tailcolour<-dist(subset(tcs.all,"zztailcolour")[,c('u','s','m','l')])


out <- list(Euc.wingblack,Euc.wingcolour,Euc.tailblack,Euc.tailcolour)

out2 <- unlist(out)
Eucledian.distance<-c(out2,(mean(out2)))
export<-as.data.frame(Eucledian.distance, row.names = c("Wing_Black","Wing_Colour","Tail_Black","Tail_Colour", "Average"), optional = FALSE)
export
one<-getwd()
two<-sub("C:/Users/Ryan/Desktop/PhD/Side Projects/Kira - comparative analysis/Real Data Files/Species_Pairs/39_American-redstart_HoodedWarbler/","39_American-redstart_HoodedWarbler",one) 
