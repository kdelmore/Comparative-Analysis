...
spp6<-gsub(".\\d+.[A-z]+_\\d+\\.6$",'.zzwing',spp5,perl=T)
spp7<-gsub(".\\d+.[A-z]+_\\d+",'.zzcrown',spp6,perl=T)
...
sppspec<-aggspec(mspecs,by=spp7,FUN=mean)
round(sppspec[1:5,],2) #just to make sure it worked out
explorespec(sppspec,by=7,lwd=3)
...
Euc.wing<-dist(subset(tcs.all,"zzwing")[,c('u','s','m','l')])

#string all of the values together 
out <- list(Euc.crown,Euc.back,Euc.rump,Euc.tail,Euc.throat,Euc.chest,Euc.wing)
...
export<-as.data.frame(Eucledian.distance, row.names = c("crown","back","rump","tail","throat","chest","wing","Average"), optional = FALSE)
export
one<-getwd()
